...
int search_min(int time[],int k){
    ...
    int i;
    for(i=0;i<k;i++){if(time[i]<min){min=time[i];index=i;}}
    return index;
}
...
int closing_time(int time[],int k){
    int max=time[0];
    int i;
    for(i=0;i<k;i++){if(time[i]>max){max=time[i];}}
    return max;
}
int waiting_time(int k,int index[],int seq[k][100]){
    int i,j,time=0;
    for(i=0;i<k;i++){
        for(j=0;j<index[i];j++){time+=(seq[i][j])*(index[i]-j-1);}
}
return time;
}
...
int main(){
    ...
    int index[k];
    int time[k];
    for(int i=0;i<k;i++){index[i]=0;time[i]=0;}
    int seq[k][100];
