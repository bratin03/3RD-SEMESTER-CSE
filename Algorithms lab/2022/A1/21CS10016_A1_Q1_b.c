...
int main()
{
  ...
  if(aux_arr[j]>=arr[i]){
   ...
   j++;}//increamenting j for checking for next element on auxillary area
  else i++;}//increamenting i for checking for next element on main area
  if(max_dist<=0)printf("None\n");//no solution found
  else printf("Maximum number of hops required: %d\n",max_dist);
  return 0;
}
