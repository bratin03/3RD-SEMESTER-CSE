...
//i.e colour 1 > colour 2 > colour 3 > colour 4
    ...
    for(i=1; i<=number; i++)
    {
        ...
        while(p!=NULL)
        {
            if(visited[p->num])
            {
                if(colour[p->num]==1)key1=0;
                else if(colour[p->num]==2)key2=0;
                else if(colour[p->num]==3)key3=0;
                else if(colour[p->num]==4)key4=0;
            }
            p=p->next;
        }

        if(key1)colour[i]=1;
        else if(key2)colour[i]=2;
        else if(key3)colour[i]=3;
        else if(key4)colour[i]=4;
