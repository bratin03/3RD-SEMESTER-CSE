...
//checking for 1
if(a[i]==1){
lflag=0;rflag=0;lmin=i,rmin=n;
for(j=i-1;(i-j)<=d&&j>=0;j--){//checking 0 at max distance on left
if (a[j]==0){lflag=1;lmin=min(i,j);}}
if(lflag==1){count+=1;a[lmin]=-1;}
else{//if no 0 on left we check on right
for(j=i+1;(j-i)<=d&&j<n;j++){
if (a[j]==0){rflag=1;rmin=j;break;}}
if(rflag==1){count+=1;a[rmin]=-1;}
}
}
}
