    ...
    for(a=1;a<=n;a++){
        for(b=a+1;b<=n;b++)
        {
            for(c=b+1;c<=n;c++){
                for(ind=0;ind<n;ind++){
                    if(arr[ind]==c){i=ind;}
                    else if(arr[ind]==b){k=ind;}
                    else if(arr[ind]==a){j=ind;}
...
int method3(int *arr,int n){
    ...
    for(i=0;i<n;i++){
        ...
        if(s->arr[s->top]!=arr[i])return 0;
        pop(s);
    }return 1;

}
void show_ans(int key){
    if(key)printf("Algolicious\n");
    else printf("Unalgolicious\n");
}
...
printf("Method 0:");show_ans(ans1);
printf("Method 1:");show_ans(ans2);
printf("Method 2:");show_ans(ans3);
printf("Method 3:");show_ans(ans4);
}